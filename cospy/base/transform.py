...
from cospy.plotting import plot_transform
try:
    import pysparse
except:
...
class WaveletTransformBase(with_metaclass(MetaRegister)):
    ...
    Available transforms are define in 'cospy.transform'.
    """
    ...
    def _set_data(self, data):
        ...
        if self.is_decimated and not (data.shape[0] // 2**(self.nb_scale) > 0):
            raise ValueError("Can't decimate the data with the specified "
                             "number of scales.")
        if isinstance(data, cospy.Image):
    ...
    def synthesis(self):
        ...
        return cospy.Image(data=data, metadata=self._image_metadata)
    ...
    def _set_transformation_parameters(self):
        ...
        flatten_fct: callable
            ...
            see 'cospy/extensions/formating.py' module for more details.
        unflatten_fct: callable
            a function used to reorganize the decomposition coefficients using
            ISAP convention, see 'cospy/extensions/formating.py' module for
            more details.
        is_decimated: bool
    ...
    def _analysis(self, data, **kwargs):
        ...
        if self.use_wrapping:
            ...
            with cospy.TempDir(isap=True) as tmpdir:
                in_image = os.path.join(tmpdir, "in.fits")
                out_mr_file = os.path.join(tmpdir, "cube.mr")
                cospy.io.save(data, in_image)
                cospy.extensions.mr_transform(in_image, out_mr_file, **kwargs)
                image = cospy.io.load(out_mr_file)
    ...
    def _synthesis(self, analysis_data, analysis_header):
        ...
        if self.use_wrapping:
            ...
            cube = cospy.Image(data=analysis_data[0], metadata=analysis_header)
            with cospy.TempDir(isap=True) as tmpdir:
                in_mr_file = os.path.join(tmpdir, "cube.mr")
                out_image = os.path.join(tmpdir, "out.fits")
                cospy.io.save(cube, in_mr_file)
                cospy.extensions.mr_recons(
                    in_mr_file, out_image, verbose=(self.verbose > 0))
                data = cospy.io.load(out_image).data
