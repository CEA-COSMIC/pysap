...
from cospy.base.transform import WaveletTransformBase
...
def load_image(image_path):
    ...
    image: Image
        the loaded image.
    """
    image = cospy.io.load(image_path)
    return image
...
def save_image(image, image_path):
    ...
    image_path: str
        the image path.
    """
    cospy.io.save(image, image_path)


def load_transform(name):
    """ Load a transformation using his name.

    All the available transfroms are stored in the 'cospy.AVAILABLE_TRANSFORMS'
    parameter.
...
def logo():
    """ COSPY logo is ascii art using fender font.
