...
infopath = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "cospy", "info.py"))
with open(infopath) as open_file:
    exec(open_file.read(), release_info)
pkgdata = {
    "cospy": [os.path.join("test", "*.py"), os.path.join("test", "*.json"),
              os.path.join("apps", "*.json")]
}
if sys.version_info >= (3, 0):
    scripts = [
        os.path.join("cospy", "apps", "cospyview3")
    ]
else:
    scripts = [
        os.path.join("cospy", "apps", "cospyview")
    ]
