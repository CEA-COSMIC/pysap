...
from pisap.plugins.mri.dictionary_learning.utils import learn_dictionary
from pisap.plugins.mri.dictionary_learning.utils import min_max_normalize
from pisap.plugins.mri.reconstruct.utils import convert_mask_to_locations
from pisap.plugins.mri.dictionary_learning.linear_DL import DictionaryLearning
from pisap.plugins.mri.dictionary_learning.utils import generate_flat_patches
from pisap.plugins.mri.dictionary_learning.reconstruct \
    import sparse_rec_condatvu
...
database_path = '/home/hc253658/STAGE/data_DL/training_database.npy'
training_set = np.load(database_path, mmap_mode="r")
...
gradient_op = GradAnalysis2(data=kspace_data,
...
x, y = sparse_rec_condatvu(gradient_op=gradient_op,
                           linear_op=linear_op,
                           std_est=None,
                           std_est_method=None,
                           std_thr=2.,
...
plt.figure('Solution in the image domain')
plt.imshow((min_max_normalize((np.abs(x)))), cmap='gray')

# solution in the sparse domain
plt.figure('Solution in the sparse domain')
plt.imshow((min_max_normalize((np.abs(linear_op.adj_op(y))))), cmap='gray')
