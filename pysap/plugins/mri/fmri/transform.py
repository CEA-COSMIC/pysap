...
class TransformT(object):
    ...
    It is composed of a 2D wavelet transform performed with sparse2d
    and a one dimensional Wavelet transform performed with pywt
    """
    def __init__(self, wavelet_name, nb_scale, wavelet_name_t=None,
        ...
        :param wavelet_name_t: str, default None
            The name of the 1D wavelet transform. Must be None or
            in pywt available transforms
        ...
        self._data_shape = None  # data shape recast in 3 dimensions

    def analysis(self, data):
        """
        Perform the analysis transform of the wavelet operator.
        :param data: np.ndarray
        The shape of data is to be [n^2, T] where T is the number of frames
        and n is the one dimensional size of each frame
        ...
        self._data_shape = (int(np.sqrt(data.shape[0])),
                            int(np.sqrt(data.shape[0])), data.shape[1])
        coeffs = []
        ...
        for t in range(data.shape[-1]):
            ...
            coeffs_, self.coeffs_shape_s = \
                flatten(self.transform_s.analysis_data)
            coeffs.append(coeffs_)
        coeffs = np.asarray(coeffs).T  # reshape coeffs
        if self.transform_t is not None:
            self.transform_t.data = coeffs
            self.transform_t.analysis()  # the analysis is done on all lines
    ...
    def synthesis(self, coeffs):
        ...
        for t in range(data_t.shape[1]):
            self.transform_s.analysis_data = unflatten(data_t[:, t],
                                                       self.coeffs_shape_s)
